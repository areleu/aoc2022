pub fn check_containment(first: &mut Vec<i32>, second: &mut Vec<i32>) -> bool {
    let (mut shortest, longest) = if first.len() > second.len() {
    ...
    shortest = shortest.into_iter().filter(|n| !longest.contains(n)).collect();
    if shortest.len() == 0 {
        true
    } else {
        false
...
fn main() {
    let string_lines = include_str!("../example.txt")
                                   .lines();
    let mut total: i32 = 0;
