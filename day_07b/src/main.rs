...
fn main() {
    let mut lines = include_str!("../input.txt").lines().collect::<Vec<&str>>().into_iter();
    // root directory
    ...
    let total_filesys = 70000000; 
    let desired = 30000000;
    let mut used_space: i64 = crawler.memory.borrow().clone().calculate_size(10000000000000000);
    folder_stack.sort_by(|a, b| a.borrow().clone().calculate_size(10000000000000000).partial_cmp(&b.borrow().clone().calculate_size(10000000000000000)).unwrap() );
    let mut filtered_map: Vec::<&Rc<RefCell<TreeNode>>> = folder_stack.iter().filter(|f| f.borrow().clone().calculate_size(10000000000000000) >= (desired - (total_filesys - used_space))).rev().collect(); 
    let mut all_sizes: Vec::<i64> = filtered_map.iter().map(|f| f.borrow().clone().calculate_size(10000000000000000)).collect(); 
    println!("{:?}", all_sizes.pop().unwrap());
}